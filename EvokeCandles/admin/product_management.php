...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_product'])) {
    $productName = $_POST['name']; // Keep original product name
    $description = $_POST['description'];
    $price = $_POST['price'];
    $images = $_FILES['images'];

    // Insert the product into the database
    $sql = "INSERT INTO products (name, description, price) VALUES ('$productName', '$description', $price)";
    if ($conn->query($sql) === TRUE) {
        ...
        foreach ($images['tmp_name'] as $key => $tmp_name) {
            if ($images['error'][$key] == 0) {
                ...
                $originalFilename = pathinfo($images['name'][$key], PATHINFO_FILENAME);
                $extension = pathinfo($images['name'][$key], PATHINFO_EXTENSION);
                
                $sanitizedName = sanitize_image_name($productName); // Use sanitized product name for images
    ...
    <section class="hero is-info">
        <div class="hero-body">
            <div class="container">
                ...
                <a class="button is-danger" href="login.php">Logout</a>
            </div>
        </div>
    </section>
